package com.example.cncn6.alarmnoti.activity.view;
...
import com.example.cncn6.alarmnoti.activity.item.SettingAlarmItem;
...
public class SettingAlarmView extends FrameLayout implements Checkable {
    ...
    private void init() {
        ...
        isSwitchOn = (Switch) findViewById(R.id.switch_alarm);
    ...
    public void setBaseAlarmData(SettingAlarmItem model, boolean isDeleteView, boolean isMedicineView, boolean isDim) {
        ...
        // 시간으로 오전/오후 나누기
        String temp = ManagerUtil.convertTimeFormat(model.getTime()); // 시간을 am/pm으로 나누는 메소드(custom)
        ...
        isSwitchOn.setChecked(model.isStatus());

        if (isDim) {
//            isSwitchOn.setSwitchToggleNotCheckedDrawableRes(R.drawable.switch_knob_dim); // custom
